...
def _month_name_es(month):
    return MONTH_NAMES_ES[month - 1].capitalize()
...
def render_tab1_summary_and_calendar(year, month, sel_center_id, employees):
    ...
    st.subheader(f"Calendario Visual - {_month_name_es(month)}")
...
def render_tab2_functions(sel_center_id):
    ...
    st.header("Gestión de Funciones")
...
def render_tab3_daily_needs(year, month, sel_center_id):
    ...
    st.info(
        f"Editando necesidades para {_month_name_es(st.session_state.get('selected_month', date.today().month))} "
        f"{st.session_state.get('selected_year', date.today().year)}"
    )
...
def render_tab4_employees(sel_center_id, employees):
    ...
    subtab1, subtab2, subtab3, subtab4 = st.tabs([
        ...
        "Registro de vacaciones/ausencias",
        "Asignación de funciones",
    ])

    with subtab1:
        st.subheader("Alta de Empleado")
        nuevo_emp = st.text_input("Nuevo Empleado", key="emp_new_emp")
        if st.button("Añadir Persona", key="emp_add_emp"):
            if not sel_center_id:
                st.warning("Selecciona un centro primero.")
            elif nuevo_emp:
        ...
        if emp_names:
            ...
            if st.button("Eliminar Empleado", key="emp_start_delete"):
            ...
            if st.session_state.pending_confirm and st.session_state.pending_delete:
                ...
                with colc1:
                    if st.button("Confirmar Eliminación", key="emp_confirm_delete"):
                        ...
                        st.rerun()
                with colc2:
                    if st.button("Cancelar", key="emp_cancel_delete"):
                        st.session_state.pending_delete = None
                        st.session_state.pending_confirm = False
                        st.rerun()
        else:
    ...
    with subtab2:
        ...
        for emp in employees:
            ...
            with col_p1:
                new_max_horas = st.number_input(
                    f"Máx Horas Anuales para {emp_name}",
                    value=float(max_horas),
                    key=f"max_horas_{emp_id}",
                )
            with col_p2:
                new_jornada = st.number_input(
                    f"Horas Jornada Diaria para {emp_name}",
                    value=float(jornada),
                    key=f"jornada_{emp_id}",
                )
    ...
    with subtab3:
        ...
        with col1:
            fecha_inicio = st.date_input("Fecha inicio", date.today(), key="vac_start")
        with col2:
            fecha_fin = st.date_input("Fecha fin", fecha_inicio, key="vac_end")
        ...
        if emp_names:
            emp_sel_name_remove = st.selectbox("Empleado", emp_names, key="remove_emp_select")
            ...
            if absences:
                ...
                selected_absence = st.selectbox("Ausencia a quitar", absence_options, key="remove_absence_select")

                if st.button("Quitar Ausencia", key="remove_vac"):
    ...
    with subtab4:
        ...
        if emp_names:
            ...
            if func_options:
                ...
                st.info("Crea funciones en la sección de Gestión de funciones primero.")
...
def render_tab5_shift_generation(year, month, sel_center_id):
    ...
    st.subheader(f"Gestión de turnos para {_month_name_es(month)} {year}")
...
def render_annual_calendar(selected_year, sel_center_id):
...
def render_annual_summary(selected_year, employees, sel_center_id):
    """Resumen anual del centro (horas y ausencias)."""
...
def render_holiday_management(sel_center_id):
    """Gestión de festivos del centro."""
    ...
    with col_h1:
        ...
        if st.button("Añadir Festivo", key="add_holiday"):
            ...
            st.rerun()
    with col_h2:
        holiday_options = [f"{d} - {n or 'Sin nombre'}" for d, n in holidays]
        if holiday_options:
            holiday_to_remove = st.selectbox("Festivo a eliminar", options=holiday_options, key="holiday_remove")
...
def render_tab6_annual_summary(selected_year, sel_center_id, employees):
    ...
    render_annual_calendar(selected_year, sel_center_id)
    st.divider()
    render_annual_summary(selected_year, employees, sel_center_id)
    st.divider()
    render_holiday_management(sel_center_id)
